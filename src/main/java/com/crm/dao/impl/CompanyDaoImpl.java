...
public class CompanyDaoImpl implements ICompanyDao {
	...
	private String SQL_TABLE_COLUMNS = "('id' int NOT NULL AUTO_INCREMENT ,"
			+ "'wechat' varchar(50) NULL, 'telphone' varchar(50), PRIMARY KEY ('id')) ";
			
	private String SQL_DB_SETTING = " ENGINE=InnoDB DEFAULT CHARACTER SET=utf8 COLLATE=utf8_general_ci ";
	private String SQL_TRANSACTION_COLUMNS = "('id'  int NOT NULL , 'member_id'  int NOT NULL , 'dept_code'  "
			+ "varchar(60) NULL ,'con_date'  datetime NULL ,"
			+ " 'order_code'  varchar(60) NULL ,'goods_code'  varchar(60) NULL ,"
			+ " 'count'  int NULL ,'total'  double NULL ,PRIMARY KEY ('id'), ";
	...
	public boolean insert(final CompanyDto companyDto) {
		try {
			...
			if(rows == 1){
				//create tables for the company
				String finalCreateMemberTableSQL = SQL_CREATE_TABLE + DBUtility.MemberTableName(companyDto.getCompanyCode())
						+ SQL_TABLE_COLUMNS + SQL_DB_SETTING;
				template.update(finalCreateMemberTableSQL);
				
				String finalCreateTransactionTableSQL = SQL_CREATE_TABLE + DBUtility.TransTableName(companyDto.getCompanyCode()) 
						+ SQL_TRANSACTION_COLUMNS 
						+ DBUtility.GenerateForeignKeyPart(DBUtility.TransTableName(companyDto.getCompanyCode()), "member_id",
								DBUtility.MemberTableName(companyDto.getCompanyCode()), "id") 
					    + SQL_DB_SETTING;
